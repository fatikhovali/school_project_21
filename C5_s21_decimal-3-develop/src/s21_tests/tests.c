...
int main() {
  ...
  Suite *s21_add, *s21_sub, *s21_div, *s21_mul, *s21_mod, *s21_is_less,
  ...
  SRunner *runner_add, *runner_sub, *runner_div, *runner_mul, *runner_mod,
  ...
  s21_mod = s21_mod_suite();
  runner_mod = srunner_create(s21_mod);
  srunner_set_fork_status(runner_mod, CK_NOFORK);
  srunner_run_all(runner_mod, CK_NORMAL);
  failed_count_mod = srunner_ntests_failed(runner_mod);
  srunner_free(runner_mod);
  ...
  int all_count =
      ...
      failed_count_mul + failed_count_mod + failed_count_is_less +
